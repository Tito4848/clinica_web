                ...
                <form method="POST" action="{{ route('citas.store') }}" id="formCita" class="glass-form" data-aos="fade-up">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select form-select-lg" id="doctor_id" name="doctor_id" required>
                            ...
                            @php
                                $doctores = \App\Models\Doctor::with('user')->get();
                            @endphp
                        ...
                        <small class="text-muted">Seleccione un doctor para ver sus horarios disponibles</small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select form-select-lg" id="hora" name="hora" required disabled>
                            <option value="" disabled selected>Primero seleccione un doctor y una fecha</option>
                        </select>
                        <small class="text-muted" id="horarios-info">Las horas disponibles aparecerán después de seleccionar doctor y fecha</small>
                    </div>
    ...
    <script>
        ...
        function cargarHorarios() {
            ...
            if (!doctorId || !fecha) {
                horaSelect.innerHTML = '<option value="" disabled selected>Primero seleccione un doctor y una fecha</option>';
                horaSelect.disabled = true;
                horariosInfo.textContent = 'Las horas disponibles aparecerán después de seleccionar doctor y fecha';
                return;
            }

            horaSelect.innerHTML = '<option value="">Cargando horarios...</option>';
            horaSelect.disabled = true;
            horariosInfo.textContent = 'Cargando horarios disponibles...';

            fetch(`{{ route('citas.horarios-disponibles') }}?doctor_id=${doctorId}&fecha=${fecha}`)
                .then(response => response.json())
                .then(horas => {
                    horaSelect.innerHTML = '';
                    if (horas.length === 0) {
                        horaSelect.innerHTML = '<option value="" disabled>No hay horarios disponibles para esta fecha</option>';
                        horariosInfo.textContent = 'No hay horarios disponibles. Por favor seleccione otra fecha.';
                        horaSelect.disabled = true;
                    } else {
                        horas.forEach(hora => {
                        ...
                        horariosInfo.textContent = `${horas.length} horario(s) disponible(s)`;
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    horaSelect.innerHTML = '<option value="" disabled>Error al cargar horarios</option>';
                    horariosInfo.textContent = 'Error al cargar horarios. Por favor intente nuevamente.';
                    horaSelect.disabled = true;
                });
        }
